...
my $FPBIN = "$FPROOT/bin";
my $TRIMMOMATIC="/mrm/bin/Fast-Plast_development/Fast-Plast/bin/Trimmomatic-0.39//trimmomatic-0.39.jar"; #path to trimmomatic executable
my $BOWTIE2="/mrm/bin/Fast-Plast_development/Fast-Plast/bin/bowtie2-2.4.2-linux-x86_64/bowtie2"; #path to bowtie2 executable
my $SPADES="/mrm/bin/Fast-Plast_development/Fast-Plast/bin/SPAdes-3.15.0-Linux/bin/spades.py"; #path to spades executable
my $BLAST="/mrm/bin/Fast-Plast_development/Fast-Plast/bin/ncbi-blast-2.11.0+/bin/"; #path to blast executable
my $SSPACE="/mrm/bin/Fast-Plast_development/Fast-Plast/bin/sspace_basic-2.1.1//SSPACE_Basic.pl"; #path to sspace exectuable
my $BOWTIE1=""; #path to bowtie1 executable
my $JELLYFISH="/mrm/bin/Fast-Plast_development/Fast-Plast/bin/jellyfish-2.3.0/bin/jellyfish"; #path to jellyfish2 excecutable
...
if($subsample){
	...
	if(@p1_array){
		...
		for my $p1f (@p1_array){
			...
			my $tfile;
			my $startline=1;
			my $endline= $subsample*4;
			if($p1f =~ /\.gz/){
				#$tfile = IO::Uncompress::Gunzip->new($p1f) or die "Problem found with $p1f: $GunzipError";
    				#while(my $line = $tfile->getline()){
				open my $tfile, '<:gzip', $p1f or die "Problem found with $p1f: $!";
				while(<$tfile>){	
					if($count <= $subsample*4){	
    						print $sub_p1 "$line";
					}
					$count++;
				}
		...
		for my $p2f (@p2_array){
			...
			my $startline=1;
                        ...
                        #	$tfile = IO::Uncompress::Gunzip->new($p2f) or die "Problem found with $p2f: $GunzipError";
                                open my $tfile, '<:gzip', $p2f or die "Problem foudn with $p2f: $!";
                                #while (my $file = $tfile->getline()){
				while(<$tfile){
					if($count <= $subsample*4){ 
                                        	print $sub_p2 "$line";
					}
...
sub run_afin {
	...
	my $afin_exec;
	if($_[6]){
		 $afin_exec = $AFIN_DIR . "/afin -c " . $_[4] . " -r ../2_Bowtie_Mapping/map_pair_hits.*.fq -l " . $_[0] . " -f .1 -d " . $_[1] . " -x " . $extension . " -p " . $_[2] . " -i " . $_[3] ." -o ". $name . "_afin --no_fusion";
	}
	else{	
		 $afin_exec = $AFIN_DIR . "/afin -c " . $_[4] . " -r ../2_Bowtie_Mapping/map_pair_hits.*.fq -l " . $_[0] . " -f .1 -d " . $_[1] . " -x " . $extension . " -p " . $_[2] . " -i " . $_[3] ." -o ". $name . "_afin";
	}
