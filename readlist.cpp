...
Readlist::Readlist( string readsfiles ){
...
bool Readlist::cmp_rc( const int ind1, const int ind2 ){
  string str1 = rlist[ind1].substr( rlist[ind1].length()-max_sort_char, max_sort_char );
  str1 = revcomp( str1 );
  string str2 = rlist[ind2].substr( rlist[ind2].length()-max_sort_char, max_sort_char );
  str2 = revcomp( str2 );
...
bool Readlist::cmp_read( const string str1, const string str2 ){
...
void Readlist::create_read_range(){
  string current( rlist[0].substr( 0, max_sort_char ) );
  long curr_start = 0;
  ...
  // add the last entry
  read_range.insert( { current, make_tuple( curr_start+1, rlist.size(), 0, 0 )});

  string rc( rlist[0].substr( rlist[0].length() - max_sort_char, max_sort_char ) );
  current = revcomp( rc );
  curr_start = 0;
...
void Readlist::add_reads( string readsfiles ){
  stringstream ss;
  ss.str( readsfiles );
  string filename;
  string buffer("");
...
tuple<long,long,long,long> Readlist::get_read_range( string read_seg ){
  try{
...
string Readlist::get_read( int ind ){
  return rlist[ind];
}

// return revcomp string of rlist member referenced by rc_reflist[ind]
string Readlist::get_rc_read( int ind ){
